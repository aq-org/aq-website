...
int AqvmMemoryRegister_IntStore(int value, struct AqvmMemoryRegister_Register* register, size_t index){
  if(register->size < index * sizeof(uint32_t)){
    // TODO(ERROR): Out of Memory.
    return -1;
  }
  register->value[index] = (uint32_t)value;
  return 0;
}
...
int AqvmMemoryRegister_DoubleStore(double value, struct AqvmMemoryRegister_Register* register, size_t index){
  if(register->size < (index + 1) * sizeof(int)){
    // TODO(ERROR): Out of Memory.
    return -1;
  }
  register->value[index] = (uint32_t)value;
